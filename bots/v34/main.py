...
class BuilderAgent:
    ...
    def _new_explore_target(self, ct: Controller, pos: Position) -> None:
        ...
        dx, dy = d.delta()
        max_range = max(self.w, self.h) // 2
        dist = random.randint(8, max(9, max_range))
        ...
        for t in ct.get_nearby_tiles():
            ...
            for d in DIRS:
                ...
                if not ib(ct, gp) or not ct.is_in_vision(gp):
                    continue
                if ct.get_tile_building_id(gp) is not None:
                    continue
                if pos.distance_squared(gp) > GameConstants.ACTION_RADIUS_SQ:
                    continue
                facing = toward(gp, self.enemy_core)
                if facing == d.opposite():
                    continue
                ti, _ = ct.get_global_resources()
                sent_cost, _ = ct.get_sentinel_cost()
                if ti >= sent_cost and ct.can_build_sentinel(gp, facing):
                    ct.build_sentinel(gp, facing)
                    return True
    ...
    def _do_explore(self, ct: Controller, pos: Position) -> None:
        ...
        if adj:
            ...
            ti, _ = ct.get_global_resources()
            ti_harv, _ = ct.get_harvester_cost()
            if ti < ti_harv + CONV_TI_THRESHOLD:
                self.state = RETURN
            else:
        ...
        if self.target:
            ti, _ = ct.get_global_resources()
            ti_harv, _ = ct.get_harvester_cost()
            use_conv = ti >= ti_harv + CONV_TI_THRESHOLD
            if use_conv:
    ...
    def _find_dead_conv(self, ct: Controller) -> Position | None:
        my = ct.get_team()
        for t in ct.get_nearby_tiles():
            bid = ct.get_tile_building_id(t)
            if bid is None or ct.get_team(bid) != my:
            ...
            for cd in CARDINALS:
                inp = t.add(cd)
                if not ib(ct, inp) or not ct.is_in_vision(inp):
                    continue
                ibid = ct.get_tile_building_id(inp)
    ...
    def _do_maintain(self, ct: Controller, pos: Position) -> None:
        ...
        if (
            self.fortify_count < MAX_FORTIFY
            and self.harvesters_built >= FORTIFY_MIN_HARVESTERS
        ):
            ti, _ = ct.get_global_resources()
            gun_cost, _ = ct.get_gunner_cost()
            if ti > gun_cost * 3:
                built = self._try_place_turret_on_network(ct, pos)
                if built:
