...
def pick_ore_target(state: State, ct: Controller) -> Position | None:
    ...
    for pos in ct.get_nearby_tiles():
        ...
        ax_ok = rnd >= 500 and rnd - state.last_ax_harvest_round >= 5
        if terrain == Environment.ORE_TITANIUM or (
            terrain == Environment.ORE_AXIONITE and ax_ok
        ):
...
def _place_harvester(state: State, ct: Controller, pos: Position) -> bool:
    if ct.can_build_harvester(pos):
        ct.build_harvester(pos)
        if state.get_env(pos) == Environment.ORE_AXIONITE:
            state.last_ax_harvest_round = ct.get_current_round()
...
def build_at_ore(state: State, ct: Controller, target_pos: Position) -> bool:
    ...
    if my_pos == target_pos:
        ...
        for d in all_dirs:
            ...
            if state.is_passable(move_pos) and ct.can_move(d):
                ct.move(d)
                _place_harvester(state, ct, target_pos)
                return True
    ...
    if my_pos.distance_squared(target_pos) <= 2:
        ...
        if has_road:
            if try_move_with_build(state, ct, target_pos):
                return True
        elif my_pos.distance_squared(target_pos) <= 1:
            _place_harvester(state, ct, target_pos)
            return True
        else:
