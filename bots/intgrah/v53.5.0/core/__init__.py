...
class Core(Unit):
    ...
    def _dump(self, ct: Controller, income_rate: float) -> None:
        ti, ax = ct.get_global_resources()
        rnd = ct.get_current_round()
        print(
            f"[core] r={rnd} ti={ti} ax={ax} "
            f"ti_del={self.ti_delivered} ax_del={self.ax_delivered} "
            f"income={income_rate:.1f} spawned={self.spawned}"
        )
