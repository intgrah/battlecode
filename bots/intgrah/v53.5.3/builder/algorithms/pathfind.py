...
class AStarSearch:
    ...
    def _run(
        ...
        if self._use_buckets:
            return self._run_buckets(state, ct, start, goal)
        return self._run_heapq(state, ct, start, goal)
    ...
    def _run_buckets(
        ...
        while emp < nb_count:
            ...
            for node_i in bucket:
                if dist[node_i] + h(Position(node_i % w, node_i // w), start) != cur_f:
                    continue
                if node_i == si:
                    return True
                if ct.get_cpu_time_elapsed() > _CPU_BUDGET:
...
move_search = AStarSearch(_MOVE_NEIGHBORS, _chebyshev, "cost_grid", use_buckets=True)
...
def conv_pathfind(
    ...
    t0 = ct.get_cpu_time_elapsed()
    result = conv_search.search(state, ct, start, target)
    t1 = ct.get_cpu_time_elapsed()
...
def conv_pathfind_blocked(
    ...
    t0 = ct.get_cpu_time_elapsed()
    result = conv_search.search_blocked(state, ct, start, goal)
    t1 = ct.get_cpu_time_elapsed()
    print(f"    conv_astar_blk={t1 - t0}us")
    return result
