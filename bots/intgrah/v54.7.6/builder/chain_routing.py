...
def _lay_segment(
    ...
    if (
        ...
        # ore in the map). A* already enforces "don't route through
        # unbuilt ore" via `buildable`, so reaching this branch with
        # path[1] on ore means a friendly road/marker is already
        # there and a conveyor replaces it cleanly.
...
def extend_step(
    ...
    Raises a `TaskRejectedError` subclass on every no-op branch;
    returns None if a conveyor/bridge was placed OR the builder
    moved this turn.
    """
    if start == target:
    ...
    with Scope("conv_astar", time=True):
