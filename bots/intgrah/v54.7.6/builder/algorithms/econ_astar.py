    ...
    def search(
            ...
            for node_i in bucket:
                ...
                for ni, extra in neighbors[node_i]:
                    ...
                    if ni != gi:
                        ...
                        if not routable[ni]:
                            continue
                        # Reachability: skip tiles definitively outside
                        # the builder's component. UF `-1` (unknown)
                        # also fails — chains run through explored
                        # territory in practice.
                        rp = reach_parent[ni]
                        if rp == -1 or _uf_find(reach_parent, ni) != my_root:
                            continue
                    nd = gn + 1 + extra
                    if nd >= dist[ni]:
                        continue
        ...
        while node != gi:
            ...
            for ni, extra in neighbors[node]:
                ...
                if ni != gi:
                    if cost_grid[ni] is INF or not routable[ni]:
                        continue
                    rp = reach_parent[ni]
                    if rp == -1 or _uf_find(reach_parent, ni) != my_root:
                        continue
