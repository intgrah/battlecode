...
from builder.extra import deny_enemy_ore, fix_enemy_conveyor
...
def _upgrade_conveyor(self: Builder, ct: Controller) -> bool:
    ...
    for bid in ct.get_nearby_buildings(2):
        ...
        if not isinstance(b, BuildingConveyor):
            continue
        if b.team != self.my_team:
            continue
        if ct.can_destroy(pos):
            ct.destroy(pos)
        if ct.can_build(EntityType.ARMOURED_CONVEYOR, pos, b.direction):
            ct.build(EntityType.ARMOURED_CONVEYOR, pos, b.direction)
            return True
    return False
...
class Builder(Unit):
    ...
    def leads_to_enemy_building(self, pos: Position) -> Position | None:
        b = self.buildings[self.idx(pos)]
        ...
        if not self.in_bounds(output_location):
            return None
        return output_location if self.is_enemy_building(output_location) else None
