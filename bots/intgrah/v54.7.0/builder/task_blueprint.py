    ...
    match entry.kind:
        case Entity.CONVEYOR:
            return (
                ...
                and bld.direction.name == entry.direction.name
            )
...
def _pick_target(
    ...
    Barriers already in vision are prioritised over all other entries.
    Within each tier, closest Chebyshev distance wins; build beats heal on ties.
    Entries with a friendly builder already adjacent are skipped.
    """
    friendly_builder_positions: frozenset[Position] = frozenset(
        ct.get_position(uid)
...
def run_blueprint(self: Builder, ct: Controller) -> bool:
    ...
    if bld is not None and bld.team != self.my_team:
        if self.my_pos == pos and ct.can_fire(pos):
            ct.fire(pos)
        else:
            make_move(self, ct, pos)
        return False
    # build action
    if self.my_pos == pos and entry.kind in _ENTITY_BLOCKED:
        move_random(self, ct)
        return True
    if chebyshev(self.my_pos, pos) <= 1:
