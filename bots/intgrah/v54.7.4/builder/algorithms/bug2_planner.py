...
def bug2_plan_iter(
    ...
    """Generator. Mutates `path_idx` in place. Yields between walker
    steps so caller can pause under a budget. Returns when the path is
    complete or no path exists.
    """
    if cost[si] is INF or cost[gi] is INF:
        return
    stride = MAX_WIDTH
    n_pad = stride * stride
    sx = si % stride
