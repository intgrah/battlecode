...
def _bfs_step(state: State, ct: Controller, target: Position) -> bool:
    w = state.w
    ...
    if state.get_cost(next_pos) > 1 and ct.can_build_road(next_pos):
        ct.build_road(next_pos)
    return try_move(ct, next_pos)


def make_move(state: State, ct: Controller, target: Position) -> bool:
    if ct.get_position() == target:
        return True
    if _bfs_step(state, ct, target):
        return True
