...
class Builder(Unit):
    ...
    def _check_dangling(self, t: Position) -> None:
        ...
        A tile `t` is dangling iff it is empty/friendly-road/dead-end-conveyor
        AND at least one of:
        - It is adjacent to an unconnected harvester.
        - Some feeder in `in_edges[t]` is active (itself has feeders or is
          adjacent to a harvester) AND unsatisfied: conveyors / armoured
          conveyors / bridges are always unsatisfied when their single output
          is `t`; splitters are unsatisfied only when `_splitter_satisfied`
          returns False.

        A friendly conveyor/armoured conveyor is only eligible when its output
        is a dead-end (OOB, wall, enemy building, barrier, launcher, harvester,
        or back-flowing conveyor). If its output is an empty buildable tile,
        that tile is the dangling end instead.

        Call after any input could have changed: in_edges[t], buildings[t],
        env[t], out_edges of t (for conveyor eligibility), a feeder splitter's
        output connectivity, or t's membership in
        ...
        if bld is None:
            if self.env[i] == Environment.WALL:
                ...
                return
        elif bld.team != self.my_team:
            self.dangling_set.discard(t)
            self.unreachable_dangling.discard(t)
            return
        elif isinstance(bld, BuildingConveyor | BuildingArmouredConveyor):
            # A conveyor is only a dangling end when its output is a dead-end
            # (OOB, wall, enemy, barrier, launcher, harvester). If its output
            # is an empty buildable tile, that tile is the dangling end instead.
            out = self.out_edges[i]
            if out:
                out_pos = out[0]
                if self.in_bounds(out_pos):
                    oi = out_pos.y * MAX_WIDTH + out_pos.x
                    out_bld = self.buildings[oi]
                    if out_bld is None:
                        if self.env[oi] != Environment.WALL:
                            self.dangling_set.discard(t)
                            self.unreachable_dangling.discard(t)
                            return
                    elif out_bld.team == self.my_team and not isinstance(
                        out_bld, BuildingBarrier | BuildingLauncher | BuildingHarvester
                    ):
                        self.dangling_set.discard(t)
                        self.unreachable_dangling.discard(t)
                        return
        elif not isinstance(bld, BuildingRoad):
            self.dangling_set.discard(t)
            self.unreachable_dangling.discard(t)
            return
        ...
        if not is_dangling:
            for feeder in self.in_edges[i]:
                fi = feeder.y * MAX_WIDTH + feeder.x
                if not self.in_edges[fi] and feeder not in self.adjacent_to_harvester:
                    continue
                fb = self.buildings[fi]
