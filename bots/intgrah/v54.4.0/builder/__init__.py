...
def _try_identify_map(self: Builder) -> KnownMap | None:
    """Every map is uniquely identified by (w, h, my_core). CANDIDATES
    is keyed on both team-A and team-B core positions, so the lookup
    works regardless of which team this builder is on.
    """
    return CANDIDATES.get((self.w, self.h, self.my_core))
...
class Builder(Unit):
    ...
    def post_init(self, ct: Controller) -> None:
        ...
        # Identify the map from (w, h, my_core). All 68 sprint-5 maps
        # are uniquely identified by this key; CANDIDATES is keyed on
        # both team-A and team-B cores so both teams match.
        self.known_map: KnownMap | None = _try_identify_map(self)
            ...
            for y in range(h):
                ...
                for x in range(w):
                    ...
                    if e == Environment.WALL:
                        cost_grid[row + x] = INF
                        conveyor_cost_grid[row + x] = INF
                    else:
                        cost_grid[row + x] = 3  # ROAD_COST
                        conveyor_cost_grid[row + x] = (
                            1 if e == Environment.EMPTY else 10
                        )
            # Rebuild pnb for every wall tile to remove it from its
            # neighbours' pnb lists. Any other tile keeps its current
            # pnb (all passable neighbours in actual-map bounds from
            # the earlier boundary fix).
            for y in range(h):
                row = y * W
                for x in range(w):
