...
def _enemy_healer_near(self: Builder, pos: Position) -> bool:
    return any(p.distance_squared(pos) <= 2 for p in self.enemy_bots)


def _friendly_bot_adjacent(self: Builder, pos: Position) -> bool:
...
def _pick_conveyor_target(
    ...
    for pos in self.nearby_buildings:
        ...
        if pos in self.all_bots and self.all_bots[pos] != self.my_id:
            continue
        if _enemy_healer_near(self, pos):
            continue
...
def _pick_attack_destination(
    ...
    *,
    ...
    for d in DIR4:
        ...
        if avoid_healers and _enemy_healer_near(self, pos):
            continue
...
def _gunner_chain_facing(self: Builder, pos: Position) -> Direction | None:
...
def run_attack(self: Builder, ct: Controller) -> None:
    ...
    if len(vulnerable_harvesters) > 0:
        ...
        sorted_harvesters = sorted(
            ...
            key=self.my_pos.distance_squared,
        )
        target = None
        for h in sorted_harvesters:
            if not _enemy_healer_near(self, h) and not _friendly_bot_adjacent(
                self,
                h,
            ):
                target = h
        ...
        if target is None:
            for h in sorted_harvesters:
                if not _enemy_healer_near(self, h):
                    target = h
                    break
        if target is None:
            target = closest(self.my_pos, vulnerable_harvesters)
        ...
        if self.my_pos.distance_squared(target) == 1 and not on_friendly_conveyor:
            if self.is_enemy_building(self.my_pos):
                ...
                if being_healed:
                    ...
                    alt = _pick_attack_destination(self, target)
            ...
            if (
                ...
                self.offense_target
                and self.offense_target.distance_squared(self.my_pos) < 20
            ):
