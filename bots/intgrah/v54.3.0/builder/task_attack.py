...
def should_attack(state: Builder, ct: Controller, pos: Position) -> bool:
...
def run_attack(state: Builder, ct: Controller) -> None:
    team = ct.get_team()
    ...
    if len(vulnerable_harvesters) > 0:
        ...
        and isinstance(
            rl := state.get_building(state.offense_launcher),
            BuildingLauncher,
        )
