...
class BuilderBot:
    ...
    def run(self, ct: Controller) -> None:
        ...
        if self.is_raider and rnd >= ATTACK_ROUND:
            self._raid(ct)
    ...
    def _raid(self, ct: Controller) -> None:
        ...
        for eid in ct.get_nearby_entities():
            ...
            if etype in (
                ...
                if pos == epos or pos.distance_squared(epos) == 0:
                    ct.self_destruct()
                    return
                bugnav_step(ct, epos, None, self.nav_state)
                return
