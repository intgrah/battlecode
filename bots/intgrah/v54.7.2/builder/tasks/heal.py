"""Restore HP on a damaged friendly building or builder bot. Tries
buildings first via `_heal_buildings` (deconfliction-aware target pick,
then walk + heal), falls back to `_heal_builders` (heal an adjacent
friendly bot, or heal self after stepping off an enemy tile). Distinct
from the free end-of-turn healing hook — this one consumes the action
cooldown.
"""
...
class TaskRejectedNothingToHealError(TaskRejectedError):
    @override
...
def _heal_buildings(self: Builder, ct: Controller) -> bool:
    """Heal a damaged friendly building. Picks a `repair_pos` (committed
    target), walks toward it, and tries `try_heal` on the best tile in
    range both before and after the move. Mutates `self.repair_pos` and
    `self.repaired_prev` for cross-turn continuity.
    """
...
def _has_wounded_enemy(self: Builder, position: Position) -> bool:
...
def _heal_adjacent_builder(self: Builder, ct: Controller) -> bool:
    """Heal the first damaged friendly builder bot within action range,
    skipping bots standing on a wounded enemy tile (we'd rather they
    finish the kill than be patched up).
    """
    ...
    for eid in adjacent_builders:
        if (ct.get_hp(eid) <= ct.get_max_hp(eid) - 4) and ct.get_team(
            ...
            if _has_wounded_enemy(self, position):
                continue
...
def _heal_self(self: Builder, ct: Controller) -> bool:
    """Heal own tile; if standing on an enemy building, step off first
    (otherwise the heal is wasted on the enemy structure too).
    """
    if ct.get_hp() > ct.get_max_hp() - GameConstants.HEAL_AMOUNT:
        return False

    if not _has_wounded_enemy(self, self.my_pos):
        try_heal(self, ct, self.my_pos, conserve_ti=False)
        move_random(self, ct)
        return True

    for d, n in self.dir_neighbours_8:
        if ct.can_move(d) and not _has_wounded_enemy(self, n):
            ct.move(d)
...
def _heal_builders(self: Builder, ct: Controller) -> bool:
    """Heal a friendly builder bot. Bails early if standing on an
    enemy tile we're trying to destroy (low-HP cases — leave us alone,
    we'll die productively); otherwise try adjacent-builder heal then
    self-heal.
    """
    ...
    if b and b.team != self.my_team:
        ...
        if self.hp[i] <= 6 and ct.get_hp() > 18:
            return False
    return bool(_heal_adjacent_builder(self, ct) or _heal_self(self, ct))


def heal(self: Builder, ct: Controller) -> None:
    if not (_heal_buildings(self, ct) or _heal_builders(self, ct)):
        raise TaskRejectedNothingToHealError
