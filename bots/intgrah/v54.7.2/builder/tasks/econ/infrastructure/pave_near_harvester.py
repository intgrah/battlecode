...
def _adjacent_harvester_ores(self: Builder, pos: Position) -> list[Position]:
    """Ore tiles whose cardinal includes `pos` AND host a friendly
    harvester. Used to compute the union of I/O exclusions from every
    harvester that shares `pos` as a cardinal."""
    out: list[Position] = []
    for d in DIR4:
        n = pos.add(d)
...
def pave_near_harvester(self: Builder, ct: Controller) -> None:
    for pos in self.nearby_tiles:
        ...
        ore_tiles = _adjacent_harvester_ores(self, pos)
