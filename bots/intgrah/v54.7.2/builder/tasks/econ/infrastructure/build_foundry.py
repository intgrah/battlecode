...
class TaskRejectedFoundryTooEarlyError(TaskRejectedError):
    ...
    def reason(self) -> Reason:
        return "round {round} < gate {gate}", {
            "round": self.current_round,
            "gate": FOUNDRY_ROUND_GATE,
        }
...
class TaskRejectedFoundryTargetWrongTypeError(TaskRejectedError):
    ...
    @override
    def reason(self) -> Reason:
        return "{target}: got {got}, expected Ti conveyor", {
            "target": self.target,
            "got": type(self.got).__name__,
        }
...
class TaskRejectedFoundryTargetEnemyError(TaskRejectedError):
    ...
    @override
    def reason(self) -> Reason:
        return "{target}: conveyor held by enemy team", {"target": self.target}
...
class TaskRejectedFoundryTerrainNotEmptyError(TaskRejectedError):
    ...
    @override
    def reason(self) -> Reason:
        return "{target}: terrain is {env}, not EMPTY", {
            "target": self.target,
            "env": self.env,
        }
...
class TaskRejectedAxChainNotConnectedError(TaskRejectedError):
    ...
    @override
    def reason(self) -> Reason:
        return "ax chain hasn't reached {target}", {"target": self.target}
