...
from visualiser import F32Grid, I16Grid, Palette, VectorField, Scalar, Tiles, PaletteStop, Colour, emit
...
P_FOG = Palette(
    stops=[PaletteStop(0.0, Colour(0, 0, 0, 0)), PaletteStop(1.0, Colour(0, 0, 0, 180))],
    special={0: TRANSPARENT},
)
P_COST = Palette(
    stops=[PaletteStop(0.0, Colour(50, 200, 50, 140)), PaletteStop(1.0, Colour(200, 50, 50, 140))],
    special={-1: TRANSPARENT},
)
P_BOOL = Palette(
    stops=[PaletteStop(0.0, Colour(0, 0, 0, 0)), PaletteStop(1.0, Colour(200, 0, 0, 140))],
    special={0: TRANSPARENT},
)
P_GREEN = Palette(
    stops=[PaletteStop(0.0, Colour(0, 0, 0, 0)), PaletteStop(1.0, Colour(0, 200, 0, 160))],
    special={0: TRANSPARENT},
)
P_RED = Palette(
    stops=[PaletteStop(0.0, Colour(0, 0, 0, 0)), PaletteStop(1.0, Colour(200, 0, 0, 160))],
    special={0: TRANSPARENT},
)
P_FLOW = Palette(
    stops=[],
...
def dump(self: Builder, _ct: Controller) -> None:
    ...
    emit(
        ...
        unconnected_harvester=Tiles(
            [(self.pos(p).x, self.pos(p).y) for p in self.adjacent_to_unconnected_harvester],
        ),
