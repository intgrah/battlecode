...
from .algorithms.pathfind import conv_pathfind, pathfind_blocked
...
def build_at_ore(state: State, ct: Controller, target_pos: Position) -> bool:
    ...
    #    # conveyor placement — all cheaply destroyable.
    #    if b is None or isinstance(b, BuildingRoad):
    ...
    if my_pos == target_pos:
        ...
        for n in unpaved_neighbors:
            if n == my_pos:
                continue
            if (
                isinstance(state.get_building(n), BuildingRoad)
                and ct.can_destroy(n)
                and can_afford(ct, EntityType.CONVEYOR)
            ):
                ct.destroy(n)
            d = n.direction_to(target_pos)
            if ct.can_build_conveyor(n, d):
                ct.build_conveyor(n, d)
                return True
    ...
    if my_pos.distance_squared(target_pos) <= 2:
        if unpaved_neighbors:
            for n in unpaved_neighbors:
                if my_pos.distance_squared(n) > 2:
                    continue
                if (
                    isinstance(state.get_building(n), BuildingRoad)
                    and ct.can_destroy(n)
            ...
            if target_has_road:
                ...
                path = pathfind_blocked(state, ct, my_pos, target_n)
                if path and len(path) > 1:
