...
def try_move_with_road(ct: Controller, target_pos: Position, state: State) -> bool:
    env = state.get_env(target_pos)
    is_ore = env in (Environment.ORE_TITANIUM, Environment.ORE_AXIONITE)
    if not is_ore and state.get_cost(target_pos) > 1 and ct.can_build_road(target_pos):
...
def try_move_adj_to(ct: Controller, target_pos: Position) -> bool:
    """Move one step so we end up cardinally/diagonally adjacent to target_pos.
    No road built."""
