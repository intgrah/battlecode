...
def is_dangling(state: State, ct: Controller, pos: Position) -> bool:
    ...
    if b is None:
        ...
        match b:
            case BuildingConveyor(direction=d) | BuildingArmouredConveyor(direction=d):
                # A friendly conveyor counts as a dangling output if its
                # output tile is empty/wall, a terminal building
                # (harvester/barrier/launcher), or a back-pointing
                # conveyor — i.e. the chain dead-ends here.
                adj = pos.add(d)
    ...
    return (
        any(
            state.conveyors_to_here[j.y * state.w + j.x]
            or j in state.adjacent_to_harvester
            for j in state.conveyors_to_here[i]
        )
        or pos in state.adjacent_to_unconnected_harvester
    )
