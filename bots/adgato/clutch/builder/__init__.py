...
from util import DIR8, can_afford, chebyshev, try_move
...
def _wander(s: State, ct: Controller) -> bool:
    # Walk away from our core on existing walkable terrain — no Ti-burning
    # road paving. Pushes idle builders toward the frontier.
    my_pos = ct.get_position()
    dirs = sorted(DIR8, key=lambda d: -chebyshev(my_pos.add(d), s.my_core))
    return any(try_move(ct, my_pos.add(d)) for d in dirs)
