...
from pathfinding import AgentState, _ALL_DIRS, _DIR_IDX, bug2_step
...
def pf_move(player: Player, ct: Controller, target: Position):
    ...
    # Stuck if current pos matches 2, 3, or 4 turns ago
    stuck = False
    for i in range(2, 5):
        if len(player.pos_history) > i and player.pos_history[-1 - i] == current:
            stuck = True
            break
    ...
    if player.stuck_count >= 16:
        ...
        for offset in [0, 1, -1, 2, -2, 3, -3, 4]:
            ...
            if current.add(d) not in visited and try_move_smart(ct, current, d):
                player.pos_history = []
                return
        # fallback: allow visited tiles if nothing else works
        for offset in [0, 1, -1, 2, -2, 3, -3, 4]:
            d = _ALL_DIRS[(gi + offset) % 8]
