...
def run_core(player: Player, ct: Controller) -> None:
    ...
    bridge_damaged = best_bridge is not None and ct.get_hp(best_bridge) < ct.get_max_hp(best_bridge)

    print(f"resource turn {rnd - player.last_resource_turn} bridge_destroyed {bridge_destroyed} bridge_damaged {bridge_damaged}")
    # Spawn a builder if bridge has had no resources for 5+ turns, bridge was destroyed, or bridge is damaged
    if player.nearest_bridge_id is not None and (rnd - player.last_resource_turn >= 5 or bridge_destroyed or bridge_damaged):
        for dx in range(-1, 2):
            for dy in range(-1, 2):
