...
def _make_goals(state: State, kind: SearchKind) -> set[int]:
    match kind:
        case SearchKind.MIXED | SearchKind.BRIDGE:
            goals = set(state.my_core_tiles)
            for i in state.my_transport:
                if not state.flow.blocked[i] and state.flow.my_frac[i] > 0.5:
                    goals.add(i)
            # Include meeting points so chains route toward them
            goals |= state.chain_plans
            return goals
