...
from marker import MarkerEureka, MarkerOpeningBook, MarkerRole
...
TASK_FNS: dict[Task, TaskFn] = {
    ...
    Task.EXTEND_CHAIN: extend_chain,
}
...
class Builder(Unit):
    ...
    def run(self, ct: Controller) -> None:
        ...
        if DEBUG_DUMP:
            dump(s, ct)
        s.claim = None
        s.pending_chain_plan = None
        s.pending_explore_claim = None
    ...
    def _write_marker(self, ct: Controller) -> None:
        ...
        elif s.pending_chain_plan is not None:
            marker_val = s.pending_chain_plan.encode()
            s.pending_chain_plan = None
        elif s.pending_explore_claim is not None:
            marker_val = s.pending_explore_claim.encode()
            s.pending_explore_claim = None
...
def _policy(state: State) -> list[tuple[float, Task]]:
    ...
    scores.append((140.0 if state.chain_plans else 0.0, Task.EXTEND_CHAIN))
    scores.append((100.0, Task.HARVEST_TI))
    scores.append((96.0 if state.en_transport else 0.0, Task.ATTACK))
    defend_core_score = 0.0
    if state.en_turrets or state.en_transport or state.en_harvesters:
        defend_core_score = 50.0  # below EXPLORE early, above PATROL
    scores.append((defend_core_score, Task.DEFEND_CORE))
