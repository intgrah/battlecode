...
from marker import CHAIN_PLAN_TTL, EXPLORE_CLAIM_TTL, MarkerChainPlan, MarkerExploreClaim, MarkerEureka, MarkerRole, MarkerTaskClaim, is_stale
from marker import decode as decode_marker
from .task import CENSUS_TTL, Role
...
def _update_ephemeral(state: State, ct: Controller) -> None:
    ...
    # chain_plans and explore_claims rebuilt fresh each turn from markers
    state.chain_plans = set()
    state.explore_claims = set()
            ...
            match bld:
                case BuildingMarker(team) if team == state.my_team:
                    ...
                    match msg:
                        ...
                        case MarkerChainPlan() as plan_msg:
                            if rnd - plan_msg.turn < CHAIN_PLAN_TTL:
                                state.chain_plans.add(plan_msg.tile_index)
                        case MarkerExploreClaim() as exp_msg:
                            if rnd - exp_msg.turn < EXPLORE_CLAIM_TTL:
                                state.explore_claims.add(exp_msg.tile_index)
