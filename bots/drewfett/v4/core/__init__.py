...
class Core(Unit):
    ...
    def _run_default(self, ct: Controller, rnd: int) -> None:
        ...
        # First 2 builders: no reserve, get them out ASAP
        # Builders 3-4: small reserve for one harvester + 2 conveyors
        # After that: progressively more Ti to justify another builder
        h_cost, _ = ct.get_harvester_cost()
        c_cost, _ = ct.get_conveyor_cost()
        if alive < 2:
            reserve = 0
        elif alive < 4:
            reserve = h_cost + c_cost * 2
        elif alive < 6:
        ...
        # Early game: halve the reserve to frontload builder spawning
        if rnd < 50:
            reserve //= 2
