...
def _log(msg: str) -> None:
    print(f"[RUSH] {msg}")
    print(f"[RUSH] {msg}", file=sys.stderr)
...
def rush(
    ...
    _log(f"--- rush eval at ({pos.x},{pos.y}) en_core=({en_core.x},{en_core.y}) ---")
    ...
    if gunner_hit is not None:
        ...
        _log(f"step1: flow in range at ({gx},{gy}) facing {facing.name}")
        result = _place_gunner_at(state, ct, Position(gx, gy), facing)
        if result is not None:
            return result
        # Can't afford — hold turn
        _log("step1: can't afford gunner, holding turn")
        return Direction.CENTRE, None

    _log("step1: no flow tile in gunner range of core")
    ...
    if flow_tile is not None:
        ...
        # Find siege target that is CLOSER to core than current flow
        siege = _find_siege_tile(state, fx, fy, en_core)
        if siege is not None and siege != (fx, fy):
            _log(
                f"step2: flow near core at ({fx},{fy}) d²={flow_dist}, extending to siege ({siege[0]},{siege[1]})"
            )
            ct.draw_indicator_line(pos, Position(fx, fy), 0, 255, 0)
    ...
    _log("step2.5: checking unconnected harvesters")
        ...
        if _on_enemy_half(state, hx, hy):
            ...
            if flow_val < 0.01:
                ...
                if not has_flow:
                    ...
                    if tap is not None:
                        siege = _find_siege_tile(state, tap[0], tap[1], en_core)
...
def _can_hit_core(
    ...
    """Check if a gunner at (gx,gy) can hit any core tile along a ray.

    Returns the facing direction that hits, or None.
    Gunner fires along a ray: closest non-empty tile on the line within r²=13.
    Walls block but aren't targetable. Buildings/core are targetable and blocking.
    """
    w, h = state.w, state.h
    core_tiles = set()
    for cdx in range(-1, 2):
        for cdy in range(-1, 2):
            ...
            if 0 <= cx < w and 0 <= cy < h:
                core_tiles.add((cx, cy))

    # Find which direction feeds this tile (if any)
    feed_dir = _find_splitter_dir(state, gx, gy)

    for d in _DIR8:
        # Don't face toward our ammo source
        if feed_dir is not None and d == feed_dir.opposite():
            continue
        ddx, ddy = d.delta()
        # Also skip if adjacent tile is a harvester (direct feed)
        adj_x, adj_y = gx + ddx, gy + ddy
        if 0 <= adj_x < w and 0 <= adj_y < h:
            adj_bld = state.building[adj_y * w + adj_x]
            if isinstance(adj_bld, BuildingHarvester):
                continue
        x, y = adj_x, adj_y
        while 0 <= x < w and 0 <= y < h:
            if (x - gx) ** 2 + (y - gy) ** 2 > _GUNNER_RANGE_SQ:
                break
            if (x, y) in core_tiles:
                return d
            # Wall blocks LoS
            env = state.env[y * w + x]
            if env == Environment.WALL:
                break
            # Any non-marker building blocks LoS (targetable + blocking)
...
def _find_flow_in_range(
    ...
    Checks both direct flow tiles (conveyors/harvesters) and extendable
    free tiles adjacent to flow sources.
    """
    ...
    # Check extendable free tiles first (preferred — no destruction)
    for tx, ty in _extendable_tiles(state):
        facing = _can_hit_core(state, tx, ty, en_core)
    ...
    # Also check our transport tiles that have NO outgoing flow
    # (dead-end conveyors that aren't feeding anything)
    for i in state.my_transport:
        if f.ti[i] < 0.01:
            continue
        ix, iy = i % w, i // w
        ...
        if isinstance(bld, BuildingBridge):
            continue
        # Only use if this tile's output goes nowhere useful
        match bld:
            case BuildingConveyor(direction=d) | BuildingArmouredConveyor(direction=d):
                ddx, ddy = d.delta()
                nx, ny = ix + ddx, iy + ddy
                if state.in_bounds(nx, ny):
                    ni = ny * w + nx
                    # If output tile has our building with flow, skip (active chain)
                    if f.ti[ni] > 0.01:
                        continue
            case _:
                continue
        facing = _can_hit_core(state, ix, iy, en_core)
        if facing is not None:
...
def _extend_from_flow(
    ...
    # Find siege target (closest tile in gunner range from our flow tile)
    ...
    _log(f"extend: siege target ({siege_x},{siege_y})")
    ct.draw_indicator_dot(Position(siege_x, siege_y), 255, 0, 0)

    # FlowAstar from flow tile to siege
...
def _find_siege_tile(
    ...
    for dx in range(-5, 6):
        for dy in range(-5, 6):
            ...
            if env == Environment.WALL:
                continue
            # Skip if our gunner already there
            cbld = state.building[ci]
            if isinstance(cbld, BuildingGunner) and cbld.team == state.my_team:
                continue
            # Must have a facing direction that actually hits a core tile
            if _can_hit_core(state, cx, cy, en_core) is None:
                continue
