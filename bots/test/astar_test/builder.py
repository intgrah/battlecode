...
class Builder(Entity):
    ...
    def _decide(self, pos: Position) -> Decision:
        # Tier 1: connect harvesters with excess flow
        best_excess_tile: tuple[int, int] | None = None
        best_dist = 999999
        for i in self.belief.harvester_tiles | self.belief.transport_tiles:
            if self.belief.excess[i] > 0.01:
                ...
                if dist < best_dist:
                    best_dist = dist
                    best_excess_tile = (x, y)
        if best_excess_tile is not None:
            step = self._chain_step(pos, best_excess_tile)
            if step is not None:
                ex = self.belief.excess[self.belief.idx(*best_excess_tile)]
                print(
                    f"fix: ({best_excess_tile[0]},{best_excess_tile[1]}) ex={ex:.2f} -> {step.action.name} @ {step.target}",
                )
                return step
    ...
    def _chain_step(self, pos: Position, source: tuple[int, int]) -> Decision | None:
        ...
        path = flow_astar(self.belief, sx, sy, cx, cy)
