...
from builder.helpers import can_afford, harvester_feed_cardinal, make_move, make_move_or_adjacent, on_enemy_side, ore_available, try_move_with_road
...
def find_contest_target(builder, pos, my_team):
    ...
    for d in DIR4:
        ...
        if team == my_team:
            continue
        if (kind == EntityType.ROAD or kind == EntityType.CONVEYOR or kind == EntityType.SPLITTER or kind == EntityType.BRIDGE):
            return n
    return None
...
def walk_to_ore_claim(builder, ct, target_pos):
    ...
    if builder.state.my_pos.distance_squared(target_pos) <= 2 and ((builder.building_kind[builder.idx(target_pos)] is not None) and (builder.building_kind[builder.idx(target_pos)] == EntityType.BARRIER or builder.building_kind[builder.idx(target_pos)] == EntityType.CONVEYOR or builder.building_kind[builder.idx(target_pos)] == EntityType.ARMOURED_CONVEYOR)) and ct.can_destroy(target_pos):
        args = {}
        args[str("target")] = auto_wrap_position(target_pos)
    ...
    return try_move_with_road(builder, ct, target_pos) or make_move_or_adjacent(builder, ct, target_pos)
...
def needs_harvester_guard(builder, cardinal, target, io_reserved):
    ...
    if ((kind is not None) and (kind == EntityType.CONVEYOR or kind == EntityType.ARMOURED_CONVEYOR)) and team == builder.state.my_team and not (not builder.out_edges[ci]) and builder.out_edges[ci][0] == target:
        return False
    return True
...
def place_harvester_guard(builder, ct, cardinal, target):
    ...
    if builder.building_kind[builder.idx(cardinal)] == EntityType.ROAD and ct.can_destroy(cardinal) and can_afford(builder, etype):
        ct.destroy(cardinal)
...
def _should_use_barrier(builder, guard_pos, target):
    ...
    passable = lambda p: builder.in_bounds(p) and builder.cost_grid[builder.idx(p)] != 1000000
    top_p = passable(top)
...
def clear_barriered_feed(builder, ct, target_pos):
    ...
    """
    sink = (builder.en_core_guess if (builder.symmetry is not None) else None) if on_enemy_side(builder, target_pos) else (t if ((t := builder.ti_sink) is not None) else builder.my_core)
    sink = sink
    if sink is None:
        return False
    ...
    for d in DIR4:
        ...
        if builder.env[builder.idx(c)] == Environment.WALL:
            continue
        if builder.building_kind[builder.idx(c)] == EntityType.BARRIER and builder.building_team[builder.idx(c)] == builder.state.my_team and ct.can_destroy(c):
            candidates.append(c)
    if (not candidates):
        return False
    chosen = (min(candidates, key=lambda c: c.distance_squared(sink)) if candidates else None)
    args = {}
    args[str("pos")] = auto_wrap_position(chosen)
    args[str("target")] = auto_wrap_position(target_pos)
    log("clear_barriered_feed: destroy friendly BARRIER on {pos} (last-resort feed clear for {target})", args)
    ct.destroy(chosen)
...
def step_off_and_build_harvester(builder, ct, target_pos):
    ...
    if d is None:
        return False
    if not ct.can_move(d) and builder.cost_grid[builder.idx(feed)] > 1 and can_afford(builder, EntityType.ROAD) and ct.can_build_road(feed):
        args = {}
        args[str("feed")] = auto_wrap_position(feed)
        log("step_off_and_build_harvester: paving feed {feed} for step-off", args)
        ct.build_road(feed)
        return True
    if builder.building_kind[builder.idx(builder.state.my_pos)] == EntityType.ROAD and ct.can_destroy(builder.state.my_pos):
        ...
        args[str("feed")] = auto_wrap_position(feed)
        log("step_off_and_build_harvester: destroy own ROAD at {at}, step to feed {feed}", args)
    ...
    if ct.can_move(d):
        ...
        if ct.can_build_harvester(target_pos):
            ...
            log("step_off_and_build_harvester: stepped to {feed} but can_build_harvester({target}) is False — building at {bld}", args)
        return True
    args = {}
    args[str("feed")] = auto_wrap_position(feed)
...
def adjacent_pave_targets(builder, pos):
    ...
    for tgt in [builder.ore_target, builder.ax_ore_target, builder.offensive_ore_target]:
        t = tgt
    ...
    for d in DIR4:
        ...
        if not builder.in_bounds(n):
            continue
        if builder.building_kind[builder.idx(n)] == EntityType.HARVESTER and builder.building_team[builder.idx(n)] == builder.state.my_team and builder.env[builder.idx(n)] == Environment.ORE_TITANIUM:
            out.append(n)
            continue
