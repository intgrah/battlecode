...
def is_turret(kind):
    return ((kind is not None) and (kind == EntityType.GUNNER or kind == EntityType.SENTINEL or kind == EntityType.BREACH or kind == EntityType.LAUNCHER))

def is_turret_or_transport(kind):
    return ((kind is not None) and (kind == EntityType.GUNNER or kind == EntityType.SENTINEL or kind == EntityType.BREACH or kind == EntityType.LAUNCHER or kind == EntityType.CONVEYOR or kind == EntityType.ARMOURED_CONVEYOR or kind == EntityType.SPLITTER))
...
def is_precious_friendly(kind, bteam, team):
    ...
    if bteam != team:
        return False
    return ((kind is not None) and (kind == EntityType.HARVESTER or kind == EntityType.FOUNDRY or kind == EntityType.LAUNCHER))
...
def gunner_facing(self_, position):
    ...
    for d in DIR8:
        ...
        is_enemy_gunner_or_sentinel = ((nk is not None) and (nk == EntityType.GUNNER or nk == EntityType.SENTINEL)) and (nt is not None) and nt != self_.my_team
...
def sentinel_facing(self_, ct, position):
    ...
    if (nearest is None) or position.distance_squared(nearest) > GameConstants.SENTINEL_VISION_RADIUS_SQ or not (position in self_.adjacent_to_harvester) or not self_.is_buildable(position) or is_turret_or_transport(kind) or is_precious_friendly(kind, team, self_.my_team) or not self_.in_bounds(position):
        return None
    uid = self_.all_bots.get(position)
    if uid is not None and (uid != self_.my_id):
        return None
