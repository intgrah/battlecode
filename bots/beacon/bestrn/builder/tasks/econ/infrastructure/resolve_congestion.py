...
def resolve_congestion(self_, ct):
    ...
    log(f"resolve_congestion: {len(self_.congested_junctions)} congested junctions visible", {})
    ...
    targets.__setitem__(slice(None), [__x for __i, __x in enumerate(targets) if __i == 0 or __x != targets[__i-1]])
    if (not targets):
    ...
    for feeder in targets:
        if ct.can_destroy(feeder):
            ...
            return None
    my_pos = self_.my_pos
    nearest = (min(targets, key=lambda p: (chebyshev(my_pos, p), p.y, p.x)) if targets else None)
