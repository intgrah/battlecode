    ...
    while (cur := (queue.pop() if queue else None)) is not None:
        ...
        for u in self_.in_edges[int(cur.y) * 50 + int(cur.x)]:
            if (u in visited):
                continue
            visited.add(u)
            if self_.building_kind[self_.idx(u)] == EntityType.BRIDGE and self_.building_team[self_.idx(u)] == self_.my_team:
                return u
            queue.append(u)
    return None
...
def destroy_dead_bridge(self_, ct):
    ...
    my_pos = self_.my_pos
    target = (min(self_.unreachable_dangling, key=lambda p: (chebyshev(my_pos, p), p.y, p.x)) if self_.unreachable_dangling else None)
    bridge = find_upstream_bridge(self_, target)
    if bridge is None:
        return TaskRejected.from_string(f"no bridge upstream of unreachable dangling {target!r}")
