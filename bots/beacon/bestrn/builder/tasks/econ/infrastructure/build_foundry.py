...
def build_foundry(self_, ct):
    ...
    if not is_conveyor:
        return TaskRejected.from_string(
            f"{target!r}: got {kind!r}, expected Ti conveyor"
        )
    ...
    if self_.env[self_.idx(target)] != Environment.EMPTY:
        return TaskRejected.from_string(
            f"{target!r}: terrain is {self_.env[self_.idx(target)]!r}, not EMPTY"
        )
    ...
    if uid is not None and (uid != self_.my_id):
        ...
        return None
    if self_.my_pos.distance_squared(target) <= 2 and try_place(
        self_, ct, EntityType.FOUNDRY, target, None, True
    ):
