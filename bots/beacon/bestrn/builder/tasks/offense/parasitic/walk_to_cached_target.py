...
def walk_to_cached_target(self_, ct):
    ...
    if (
        ol is not None
        and (self_.building_kind[self_.idx(ol)] == EntityType.LAUNCHER)
        and (self_.building_team[self_.idx(ol)] == self_.my_team)
        and (self_.my_pos.distance_squared(offense_target) > 8)
    ):
