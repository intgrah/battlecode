...
from builder.tasks.offense.helpers import buildable, is_allied_transport, open_tiles, pick_attack_destination, pick_harvester_target, should_attack, vulnerable_harvesters, without_allied_transport
...
def approach_harvester(self_, ct):
    ...
    if self_.my_pos.distance_squared(target) == 1 and not on_friendly_conveyor:
        return TaskRejected("already adjacent to target (fire/turret task handles this)")
    ...
    adjacent_launchers: list[object] = list((p for p in self_.neighbours_8 if self_.building_kind[self_.idx(p)] == EntityType.LAUNCHER))
    best_adjacent_launcher = closest(destination, adjacent_launchers)
    if self_.my_pos.distance_squared(destination) <= 2 or self_.my_pos.distance_squared(target) < 9:
        make_move(self_, ct, destination)
    else:
        bal = best_adjacent_launcher
        if bal is not None and (self_.is_walkable(destination)) and (bal.distance_squared(destination) <= GameConstants.LAUNCHER_VISION_RADIUS_SQ):
            pass
        else:
            loc = launcher_location
            if loc is not None and ((best_adjacent_launcher is None)) and (self_.is_walkable(destination)) and (loc.distance_squared(destination) <= GameConstants.LAUNCHER_VISION_RADIUS_SQ) and (try_place(self_, ct, EntityType.LAUNCHER, loc, None, True)):
    ...
    if cur_pos.distance_squared(target) == 1 and self_.is_enemy_building(cur_pos) and should_attack(self_, cur_pos):
