...
def push_extend(self_, ct):
    ...
    if not on_enemy_side(self_, start):
        return TaskRejected.from_string(f"dangling {start!r} is on our side of the bisector")
    resource = resource_at(self_, start)
    if resource != ResourceType.TITANIUM:
        return TaskRejected.from_string(f"dangling {start!r} is {resource!r}, push_extend is Ti-only")
