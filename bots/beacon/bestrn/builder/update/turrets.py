...
def update_enemy_turrets(builder) -> None:
    ...
    if t is not None:
        ...
        valid = (
            ...
            and (
                builder.building_kind[i] == EntityType.GUNNER
                or builder.building_kind[i] == EntityType.SENTINEL
            )
        ) and builder.building_team[i] != my_team
        if not valid:
    ...
    for pos in nearby:
        ...
        is_enemy_turret = (
            ...
            and builder.building_team[i] != my_team
            and (builder.building_team[i] is not None)
        )
        if is_enemy_turret:
