...
from builder.helpers import make_move, on_enemy_side, trace_upstream, try_move_with_road, try_place
...
def _lay_segment(builder, ct, start_pos, path):
    ...
    entity_type: EntityType | None = ((lambda b: ct.get_entity_type(b))(bid) if bid is not None else None)
    b = bid
    if (entity_type == EntityType.ROAD) and b is not None and (ct.get_team(b) != builder.state.my_team) and (ct.can_fire(start_pos)):
        args = {}
    ...
    if start_pos.distance_squared(builder.my_core) <= 5 and path[len(path) - 1] == builder.my_core:
        for d in DIR4:
    ...
    if entity_type == EntityType.CONVEYOR and ct.get_direction(bid) == (direction if direction is not None else Direction.CENTRE):
        return True
    b = bid
    ...
    destination_team: object | None = ((lambda b: ct.get_team(b))(destination_building) if destination_building is not None else None)
    destination_is_marker = ct.get_entity_type(b) == EntityType.MARKER if ((b := destination_building) is not None) else False
    d = direction
    if d is not None and (is_cardinal(d)) and ((destination_building is None) or destination_team == builder.state.my_team or destination_is_marker):
...
def extend_step(builder, ct, start, target, resource):
    ...
    if chebyshev(start, target) <= 1 and target == builder.my_core:
        return TaskRejected.from_string(f"{start!r} is already adjacent to core; nothing to lay")
    ...
    if not builder.cost_grid[builder.idx(start)] != 1000000:
        if len(existing_path) > 1:
            ...
            return TaskRejected.from_string(f"{start!r} is unpassable and no upstream fallback")
    is_ax = (resource == ResourceType.RAW_AXIONITE or resource == ResourceType.REFINED_AXIONITE)
    with Scope.new_timed("conv_astar") as _g:
        __block_value = builder.ax_conv_astar(start, target, resource) if is_ax else builder.ti_conv_astar(start, target, resource)
    path = __block_value
    path = path
    if path is None:
        fail = list(builder.ax_conv_search.last_fail_reason) if is_ax else list(builder.conv_search.last_fail_reason)
    ...
    if chebyshev(current_pos, start) <= 1:
        if len(path) < 2:
            return TaskRejected.from_string(f"in range of {start!r} but A* path is empty")
        if _lay_segment(builder, ct, start, path):
            did_something = True
    if make_move(builder, ct, start):
        did_something = True
    if not did_something:
        return TaskRejected.from_string(f"could neither lay at {start!r} nor move toward {target!r}")
    return None
...
def extend_chain(builder, ct, start):
    ...
    if on_enemy_side(builder, start):
        return TaskRejected.from_string(f"{start!r} is enemy-side; deferring to OFFENSE")
    ...
    if resource == ResourceType.RAW_AXIONITE:
        ...
        target = target
        if target is None:
            return TaskRejected.from_string(f"chain at {start!r} is Ax but ax_sink is None")
