...
from .task_fortify_ore import _best_ore as _fortify_best_ore
from .task_fortify_ore import fortify_ore
from .task_harvest_ax import harvest_ax
from .task_harvest_ti import harvest_ti
from .task_heal_core import heal_core
from .task_nav_center import nav_center
...
TASK_FNS: dict[Task, TaskFn] = {
    ...
    Task.FORTIFY_ORE: fortify_ore,
    Task.NAV_CENTER: nav_center,
}
...
class Builder(Entity):
    def __init__(self, ct: Controller) -> None:
        ...
        # First 2 bots (spawned before round 50) are SECURE (farmers).
        # The bot spawned around round 50 is the first ADVANCE (attacker).
        # After that, every ~200 rounds the core spawns another attacker.
        rnd = ct.get_current_round()
        if rnd <= 2:
            self.state.role = Role.SECURE
        elif 45 <= rnd <= 80:
            # First attacker spawned around round 50
            self.state.role = Role.ADVANCE
        elif rnd >= 200 and (rnd % 200) < 30:
            # Periodic attacker every ~200 rounds
            self.state.role = Role.ADVANCE
        else:
            self.state.role = Role.SECURE
    ...
    def _run_policy(self, ct: Controller) -> tuple[Direction, Action | None]:
        s = self.state
        fallback: tuple[Direction, Action | None] | None = None
        for _, task in _policy(s):
            ...
            if result is None:
                continue
            move, build = result
            # Prefer tasks that actually DO something (move or build)
            if move != Direction.CENTRE or build is not None:
                return result
            # Keep first waiting result as fallback
            if fallback is None:
                fallback = result
        return fallback if fallback is not None else (Direction.CENTRE, None)
...
def _policy_secure(
    ...
    has_excess = any(
        state.my_flow.excess[i] > 0.01 for i in own_harvesters | state.my_transport
    )
...
def _policy_advance(
    ...
    has_fortify_target = _fortify_best_ore(state) is not None

    scores.append((100.0 if has_fortify_target else 0.0, Task.FORTIFY_ORE))
    scores.append((20.0, Task.NAV_CENTER))
