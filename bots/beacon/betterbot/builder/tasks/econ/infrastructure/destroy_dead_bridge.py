...
class TaskRejectedNoBridgeUpstreamError(TaskRejectedError):
    ...
    @override
    def reason(self) -> Reason:
        return "no bridge upstream of unreachable dangling {dangling}", {
...
class TaskRejectedCannotActError(TaskRejectedError):
    ...
    @override
    def reason(self) -> Reason:
        return "cannot destroy or approach bridge {bridge}", {"bridge": self.bridge}
...
def destroy_dead_bridge(self: Builder, ct: Controller) -> None:
    ...
    if ct.can_destroy(bridge):
        ct.destroy(bridge)
        self.apply_local_destroy(bridge)
        return
