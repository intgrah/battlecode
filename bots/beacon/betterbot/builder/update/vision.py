...
def _apply_post_transition(
    ...
    self.update_pnb(i)
    self._check_dangling(pos, trigger)
    for feeder in self.in_edges[i]:
        ...
        if isinstance(fb, BuildingSplitter):
            for sib in self.out_edges[feeder.y * MAX_WIDTH + feeder.x]:
                if sib != pos:
                    self._check_dangling(sib, "splitter_sibling")


def apply_local_destroy(self: Builder, pos: Position) -> None:
...
def update_vision(self: Builder, ct: Controller) -> None:
    ...
    for pos in self.nearby_tiles:
        ...
        if bld_changed or env_changed:
            if bid is None:
                # Tile became empty — share the destroy path with the
                # mid-turn `apply_local_destroy` helper so any future
                # destroy-after-edit invariant work has one canonical
                # implementation.
                apply_local_destroy(self, pos)
            else:
                _remove_topology(self, pos, i)
                ...
                _apply_post_transition(self, pos, i, env, "building_changed")
