...
//!
//! Phase 5 PosInt note: Sentinel uses UnitState (not Builder), so _p accessor
//! variants are not available. The attackable-tile loop is driven by
//! ct.get_attackable_tiles() which returns Vec<Position> — all lookups go
//! through ct API anyway. TODO Phase 5 holdout: convert if UnitState gains
//! _p accessors or if attackable tiles are stored as PosInt.
...
impl Unit for Sentinel {
    ...
    fn run(&mut self, ct: &mut Controller<'_>) {
        ...
        // Cooperative targeting: within a (score, ...) priority band,
        // multiple sentinels all break ties by smallest target id, so
        // they stack damage on the same enemy instead of splitting fire.
        ...
        for tile in attackable {
            ...
            if pyrust::vec::contains!(self.state.enemy_bots, &tile) {
                ...
                let target_id = uid.unwrap_or(i32::MAX);
                if (score, -target_id) > (best_score, -best_id) {
                    best_score = score;
                    best_id = target_id;
            ...
            // Lex tiebreak: maximise score, minimise id (cooperation).
            // We negate id so the > comparison still picks "smallest id"
            // as preferred within an equal score.
            if (score, -bid) > (best_score, -best_id) {
                best_score = score;
                best_id = bid;
                best_target = Some(tile);
            }
