...
pub fn opportunistic_attack(self_: &mut Builder, ct: &mut Controller<'_>) -> TaskResult {
    ...
    // Use cached arrays to avoid 2 Python-boundary ct calls.
    // can_fire (checked above) already guarantees a building exists here,
    // so building_ids[i] / building_team[i] are always populated.
    let i = idx_of(self_.my_pos) as usize;
