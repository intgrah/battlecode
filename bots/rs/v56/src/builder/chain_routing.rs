...
fn _lay_segment(
    ...
    let entity_type: Option<EntityType> =
        pyrust::opt_map!(bid, |b| pyrust::unwrap!(ct.get_entity_type(Some(b))));
    ...
    let destination_team: Option<_> = pyrust::opt_map!(destination_building, |b| pyrust::unwrap!(
        ct.get_team(Some(b))
    ));
