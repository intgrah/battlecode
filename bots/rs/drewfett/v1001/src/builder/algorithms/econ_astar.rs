...
use crate::util::constants::{BOUND_RANGE, INF, MAX_WIDTH, STRIDE};
...
fn x_of_table() -> Vec<i32> {
    pyrust::collect!(pyrust::map!((0..BOUND_RANGE), |i| (i % STRIDE) as i32))
}
...
impl AStarSearch {
    ...
    pub fn new() -> Self {
        ...
        let mut neighbors: Vec<Vec<(i32, i32)>> = Vec::with_capacity(BOUND_RANGE);
        let mut cardinal_neighbors: Vec<Vec<i32>> = Vec::with_capacity(BOUND_RANGE);
        let mut weighted_neighbors: Vec<Vec<i32>> = Vec::with_capacity(BOUND_RANGE);
        // Stencils are PosInt-indexed (`y * STRIDE + x`). The outer loop
        // covers BOUND_RANGE = MAX_WIDTH * STRIDE entries; cells at
        // x ∈ [MAX_WIDTH, STRIDE) are unreachable holes and get empty
        // neighbour lists so callers index directly without conversion.
        for cy in 0..MAX_WIDTH as i32 {
            for cx in 0..STRIDE as i32 {
                let mut all: Vec<(i32, i32)> = pyrust::vec::new!();
                let mut card: Vec<i32> = pyrust::vec::new!();
                let mut wt: Vec<i32> = pyrust::vec::new!();
                if cx < MAX_WIDTH as i32 {
                    for &(dx, dy, extra) in &neighbors_template {
                        let nx = cx + dx;
                        let ny = cy + dy;
                        if 0 <= nx && nx < MAX_WIDTH as i32 && 0 <= ny && ny < MAX_WIDTH as i32 {
                            let ni = ny * (STRIDE as i32) + nx;
        ...
        Self {
            ...
            cardinal_neighbors,
            weighted_neighbors,
            _dist: vec![INF; BOUND_RANGE],
            dist_bwd: vec![INF; BOUND_RANGE],
            parent_fwd: vec![-1; BOUND_RANGE],
            parent_bwd: vec![-1; BOUND_RANGE],
            closed_fwd: vec![false; BOUND_RANGE],
            closed_bwd: vec![false; BOUND_RANGE],
            ...
            reach_root_cache: vec![-1; BOUND_RANGE],
    ...
    fn search_unidirectional(
        ...
        {
            for _fill_i in 0..pyrust::len!(self._dist) {
                self._dist[_fill_i] = INF;
            }
            self.target = Some(target);
        } else {
            resumed_search = true;
        ...
        // this ~2x faster on tight inner loops (intgrah's Python source uses
        // the same pattern). Native Rust compiler also benefits — borrow
        // checker is satisfied because each field is a disjoint borrow.
        let dist_arr = &mut self._dist;
        let f_at_arr = &mut self.f_at;
        let buckets_fwd = &mut self.buckets_fwd;
        let reach_root_cache = &mut self.reach_root_cache;
        let reach_root_touched = &mut self.reach_root_touched;
        let cardinal_neighbors = &self.cardinal_neighbors;
        let weighted_neighbors = &self.weighted_neighbors;
        let x_heur_fwd = &self.x_heur_fwd;
        let y_heur_fwd = &self.y_heur_fwd;
        let x_of = &self.x_of;
        let y_of = &self.y_of;
        let reach_parent = &mut ctx.reach_parent;
        while emp < nb_count {
            if pyrust::vec::is_empty!(buckets_fwd[(cur_f & bucket_mask) as usize]) {
                cur_f += 1;
                emp += 1;
                continue;
            }
            ...
            let mut idx = 0;
            while idx < pyrust::len!(buckets_fwd[slot]) {
                let node_i = buckets_fwd[slot][idx];
                idx += 1;
                if f_at_arr[node_i as usize] != cur_f {
                    continue;
                }
                nodes_expanded += 1;
                if node_i == si {
                ...
                let gn = dist_arr[node_i as usize];
                let base_nd = gn + 1;
                let weighted_nd = base_nd + 9;
                let cardinals = &cardinal_neighbors[node_i as usize];
                for &ni in cardinals {
                    if !routable[ni as usize] {
                        continue;
                    }
                    let rp = reach_parent[ni as usize];
                    if rp != my_root {
                        if rp == -1 {
                            continue;
                        }
                        let mut root = reach_root_cache[ni as usize];
                        if root == -1 {
                            root = uf_find(reach_parent, ni);
                            reach_root_cache[ni as usize] = root;
                            pyrust::vec::push!(reach_root_touched, ni);
                        }
                        if root != my_root {
                            continue;
                        }
                    ...
                    if nd >= dist_arr[ni as usize] {
                        continue;
                    }
                    dist_arr[ni as usize] = nd;
                    let nf = nd
                        + x_heur_fwd[x_of[ni as usize] as usize]
                        + y_heur_fwd[y_of[ni as usize] as usize];
                    f_at_arr[ni as usize] = nf;
                    pyrust::vec::push!(buckets_fwd[(nf & bucket_mask) as usize], ni);
                }
                let weighted = &weighted_neighbors[node_i as usize];
                for &ni in weighted {
                    if !routable[ni as usize] {
                        continue;
                    }
