...
impl DebugCtx {
    ...
    fn emit_child(&mut self, node: Value) {
        ...
        let parent = self.current_scope_mut();
        pyrust::vec::push!(parent["children"].as_array_mut().unwrap(), node);
    }
...
// `#[pyrust::context_manager]` not present in our pyrust branch — Scope
// works as a plain RAII guard in native and as an explicit construction
// + Drop call in translated Python.
pub struct Scope {
    pub label: String,
}
