...
/// WS-6: invalidation hook for the bug2 path cache.
///
/// Returns true iff every tile in `path` is in-bounds and currently passable
/// (cost != INF). Used by `nav::BugNav::step` before reusing a cached plan:
/// if any tile became impassable since cache (e.g. a new turret/wall/enemy
/// building), the cached path is stale and must be recomputed.
///
/// `stride` is the row stride matching the layout of `cost` (`MAX_WIDTH`).
#[must_use]
pub fn path_is_passable(path: &[Position], cost: &[i32], _stride: usize, w: i32, h: i32) -> bool {
    for p in path {
        if p.x < 0 || p.x >= w || p.y < 0 || p.y >= h {
            return false;
        }
        let i = idx_of(*p) as usize;
        if cost[i] == INF {
            return false;
        }
    }
...
pub struct Bug2Planner {
    ...
    h: i32,
    gi: i32,
    /// Mutated in place; exposed via `into_path_idx()`.
...
impl Bug2Planner {
    ...
    fn cw_substep(&mut self, cost: &[i32]) {
        let mut moved = false;
        for _ in 0..8 {
            self.cw_dir = ((self.cw_dir - 1) % 8 + 8) % 8;
            ...
            if cost[cell as usize] != INF {
                ...
                if IS_CARDINAL[self.cw_dir as usize] {
                    self.cw_dir = ((self.cw_dir + 2) % 8 + 8) % 8;
                } else {
                    self.cw_dir = ((self.cw_dir + 3) % 8 + 8) % 8;
                }
    ...
    fn ccw_substep(&mut self, cost: &[i32]) {
        let mut moved = false;
        for _ in 0..8 {
            self.ccw_dir = ((self.ccw_dir + 1) % 8 + 8) % 8;
            ...
            if cost[cell as usize] != INF {
                ...
                if IS_CARDINAL[self.ccw_dir as usize] {
                    self.ccw_dir = ((self.ccw_dir - 2) % 8 + 8) % 8;
                } else {
                    self.ccw_dir = ((self.ccw_dir - 3) % 8 + 8) % 8;
                }
