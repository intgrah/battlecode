...
pub fn find_contest_target(builder: &Builder, pos: Position, my_team: Team) -> Option<Position> {
    for d in DIR4 {
        ...
        let np = idx_of(n);
        let Some((kind, team)) = builder.get_building_p(np) else {
            continue;
        };
        if team == my_team {
            continue;
...
pub fn is_guarded_cardinal(builder: &Builder, pos: Position) -> bool {
    let p = idx_of(pos);
    if builder.get_env_p(p) == Some(Environment::Wall) {
        return true;
    }
    let Some(kind) = builder.kind_at_p(p) else {
        return false;
    };
...
pub fn place_harvester_guard(
    ...
    if builder.kind_at_p(idx_of(cardinal)) == Some(EntityType::Road)
...
pub fn clear_barriered_feed(
    ...
    for d in DIR4 {
        ...
        let cp = idx_of(c);
        if builder.get_env_p(cp) == Some(Environment::Wall) {
            continue;
        }
        if builder.kind_at_p(cp) == Some(EntityType::Barrier)
            && builder.team_at_p(cp) == Some(builder.state.my_team)
...
pub fn adjacent_pave_targets(builder: &Builder, pos: Position) -> Vec<Position> {
    ...
    for d in DIR4 {
        ...
        let np = idx_of(n);
        if builder.kind_at_p(np) == Some(EntityType::Harvester)
            && builder.team_at_p(np) == Some(builder.state.my_team)
            && builder.get_env_p(np) == Some(Environment::OreTitanium)
        {
