...
pub fn update_enemy_turrets(builder: &mut Builder) {
    ...
    if let Some(t) = builder.nearest_enemy_turret {
        ...
        let kind = bk[i];
        let team = bt[i];
        let valid = matches!(kind, Some(EntityType::Gunner | EntityType::Sentinel | EntityType::Launcher | EntityType::Breach))
            && team != Some(my_team)
            && pyrust::is_some!(team);
        if !valid {
    ...
    for pos in &builder.state.nearby_tiles {
        ...
        let kind = bk[i];
        let team = bt[i];
        let is_enemy_turret = matches!(kind, Some(EntityType::Gunner | EntityType::Sentinel | EntityType::Launcher | EntityType::Breach))
            && team != Some(my_team)
            && pyrust::is_some!(team);
        if is_enemy_turret {
