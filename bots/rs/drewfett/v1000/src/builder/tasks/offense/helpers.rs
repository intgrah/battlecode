...
pub fn pick_conveyor_target(
    ...
    let my_p = idx_of(my_pos);
    let ec_p = idx_of(enemy_core);
    let mut best: Option<Position> = None;
    let mut best_score: Option<(i32, i32, i32)> = None;
    for &pos in &self_.nearby_buildings {
        let pi = idx_of(pos);
        let Some((kind, team)) = self_.get_building_p(pi) else {
            continue;
        };
        ...
        if pyrust::dict::contains!(self_.attack_tile_blacklist, &pi) {
            continue;
        }
        if pyrust::set::contains!(self_.friendly_turret_ray_tiles, &pi) {
            continue;
        }
        ...
        let near_core = dist_sq(ec_p, pi) <= 25;
...
pub fn pick_attack_destination(
    ...
    for &dp in &DIR4_INT {
        let pi = target_p + dp;
        if pi < 0 || self_.posint_valid[pi as usize] == 0 {
            continue;
        }
        if !self_.is_passable_p(pi) {
            continue;
        }
        let kind = self_.kind_at_p(pi);
        let team = self_.team_at_p(pi);
        if matches!(
            kind,
            Some(
                EntityType::Conveyor
        ...
        let in_ray: i32 = i32::from(pyrust::set::contains!(self_.enemy_turret_ray_tiles, &pi));
...
pub fn gunner_chain_facing(self_: &Builder, pos: Position) -> Option<Direction> {
    ...
    for ii in 0..8usize {
        ...
        for _ in 0..4 {
            ...
            if self_.get_env_p(cp) == Some(Environment::Wall) {
                break;
            }
            let Some((kind, team)) = self_.get_building_p(cp) else {
                continue;
            };
            if team == self_.my_team {
                break;
...
fn has_open_side(self_: &Builder, position: Position) -> bool {
    ...
    for &dp in &DIR4_INT {
        ...
        let nkind = self_.kind_at_p(np);
        let nteam = self_.team_at_p(np);
        if matches!(
            nkind,
            Some(
                EntityType::Conveyor
        ...
        ) && nteam == Some(self_.my_team)
        {
            continue;
        }
        let new_position = pos_of(np);
...
pub fn vulnerable_harvesters(self_: &Builder) -> Vec<Position> {
    ...
    for &pos in &self_.nearby_buildings {
        ...
        if has_open_side(self_, pos) {
            pyrust::vec::push!(result, pos);
        }
    }
    result
}
...
pub fn begin_turn_offense(self_: &mut Builder, ct: &mut Controller<'_>) {
    ...
    for k in keys {
        ...
        if v > 1 {
            pyrust::dict::insert!(self_.attack_tile_blacklist, k, v - 1);
        } else {
            pyrust::dict::remove!(self_.attack_tile_blacklist, &k);
        }
    }
