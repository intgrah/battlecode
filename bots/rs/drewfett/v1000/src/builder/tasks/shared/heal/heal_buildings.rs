...
fn best_healable_building(self_: &mut Builder, ct: &mut Controller<'_>) -> Option<Position> {
    ...
    for pos in healable {
        ...
        let is_critical = pyrust::vec::contains!(self_.adjacent_to_harvester, &(i as i32));
    ...
    for p in hbs {
        ...
        if self_.hp[pi] < self_.max_hp[pi] {
            pyrust::vec::push!(healed_kept, p);
        }
    }
    self_.healable_buildings = healed_kept;
    best
}
...
pub fn heal_buildings(self_: &mut Builder, ct: &mut Controller<'_>) -> TaskResult {
    ...
    {
        let rp_p = idx_of(rp);
        let ti_idx = rp_p as usize;
        if let Some((_kind, team)) = self_.get_building_p(rp_p)
    ...
    let my_pos_p2 = idx_of(self_.my_pos);
    if (pyrust::is_some!(new_repair)
        && dist_sq(idx_of(pyrust::unwrap!(new_repair)), my_pos_p2) <= 2)
