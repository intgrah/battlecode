...
/// PosInt-native chebyshev. Module-level table accessed via
/// `util::posint::cheb`; no per-Builder table needed.
#[must_use]
#[inline]
pub fn chebyshev_p(p: PosInt, q: PosInt) -> i32 {
    posint_cheb(p, q)
}

/// PosInt-native manhattan.
#[must_use]
#[inline]
pub fn manhattan_p(p: PosInt, q: PosInt) -> i32 {
    posint_manhat(p, q)
}

/// PosInt-native squared euclidean.
#[must_use]
#[inline]
pub fn euclidean_sq_p(p: PosInt, q: PosInt) -> i32 {
    posint_dist_sq(p, q)
}
...
{
    let my_d = chebyshev_p(my_p, target_p);
    for (fb_p, fb_id) in friendlies {
        let fb_d = chebyshev_p(fb_p, target_p);
        if fb_d < my_d || (fb_d == my_d && fb_id < my_id) {
            return false;
        }
    }
