...
pub fn update_patrol(builder: &mut Builder) {
    ...
    let mut args = Map::new();
    args.insert("own".to_string(), Value::Number(own_count.into()));
    args.insert("trans".to_string(), Value::Number(transitive_count.into()));
    args.insert("friend".to_string(), auto_wrap_position(best));
    args.insert("d".to_string(), Value::Number(best_d.into()));
    args.insert("nf".to_string(), Value::Number(nf.into()));
    log(
