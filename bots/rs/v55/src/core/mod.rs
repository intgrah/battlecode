...
impl Core {
    ...
    fn should_spawn(&self, ct: &mut Controller<'_>, income_rate: f64) -> bool {
        ...
        let income_threshold = (Self::INCOME_PER_UNIT * live + Self::INCOME_QUADRATIC_TERM * live * live)
            / self.spawn_tempo;
        let has_income = income_rate * 4.0 > income_threshold;
        let surplus_threshold = (pyrust::float!(Self::SURPLUS_SCALE_FACTOR) * (pyrust::unwrap!(ct.get_scale_percent()) / 100.0) + pyrust::float!(Self::SURPLUS_BASELINE))
