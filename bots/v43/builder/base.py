...
class BuilderBase(Entity):
    ...
    def _cardinal_adjacent(self, pos: Position, target: Position) -> Position | None:
        """Find the best cardinal neighbor of target to stand on.

        Prefers already-walkable tiles (roads/conveyors) over empty ground,
        and closer tiles over farther ones.
        """
        best = None
        best_cost = COST_IMPASSABLE
        best_dist = 999999
        for ddx, ddy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ax, ay = target.x + ddx, target.y + ddy
            if not self.belief.in_bounds(ax, ay):
                continue
            cost = self.belief.walkable(ax, ay)
            if cost >= COST_IMPASSABLE:
                continue
            dist = (pos.x - ax) ** 2 + (pos.y - ay) ** 2
            # Prefer walkable (cost < COST_EMPTY) then by distance.
