...
class HarvestMixin(BuilderBase):
    ...
    def _harvest_impl(
        ...
        # Immediately build harvester if adjacent to unharvested ore.
        ...
        # Sort by distance to core (prioritize closer ore for shorter chains),
        # then by distance to builder (among equally close ore, pick nearest).
        candidates = sorted(
            unharvested,
            key=lambda o: (
                (cx - o[0]) ** 2 + (cy - o[1]) ** 2,
                (pos.x - o[0]) ** 2 + (pos.y - o[1]) ** 2,
            ),
        )
