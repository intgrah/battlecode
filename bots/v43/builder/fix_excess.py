...
class FixExcessMixin(BuilderBase):
    ...
    def _fix_excess_ti_rax(
        ...
        for i in (
            ...
            total_ex = ti_ex + rax_ex
            if total_ex > 0.01 and not self._is_claimed(i, TaskKind.FIX_EXCESS):
                x, y = i % w, i // w
                dist = (pos.x - x) ** 2 + (pos.y - y) ** 2
                candidates.append((total_ex, dist, (x, y)))
        if not candidates:
            return None
        # Pick highest excess first; break ties by distance to builder.
    ...
    def _fix_excess_ax(
        ...
        for i in self.belief.my_harvesters | self.belief.my_transport:
            ax_ex = f.ax_excess[i]
            if ax_ex > 0.01 and not self._is_claimed(i, TaskKind.FIX_EXCESS):
                x, y = i % w, i // w
                dist = (pos.x - x) ** 2 + (pos.y - y) ** 2
                candidates.append((ax_ex, dist, (x, y)))
        if not candidates:
            return None
        candidates.sort(key=lambda c: (-c[0], c[1]))
        best_tile = candidates[0][2]
        ...
        # Try routing ax to a foundry first (for refined ax), else to ti conveyor.
        target = self._find_foundry_input(best_tile)
        if target is None:
            target = self._find_ti_conveyor(best_tile)
        if target is None:
            # No foundry or ti conveyor: route to core.
            return self._build_chain_to_core(
                ct,
                pos,
                best_tile,
    ...
    def _find_foundry_input(self, source: tuple[int, int]) -> tuple[int, int] | None:
        """Find the nearest foundry that could accept more ax input."""
        best = None
        best_dist = 999999
        w = self.belief.w
        f = self.belief.my_flow
        for i in self.belief.my_foundries:
            # Foundry wants balanced ti/ax input. Pick one with ti > ax.
            if f.ti[i] <= f.ax[i]:
                continue
            x, y = i % w, i // w
