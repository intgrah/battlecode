from cambc import Controller, Direction, EntityType, Position

from .base import BuilderBase
from .build import Build, BuildKind

_RAIDABLE = frozenset((EntityType.CONVEYOR, EntityType.BRIDGE, EntityType.SPLITTER))


class RaidMixin(BuilderBase):
    def _raid(
        self,
        ct: Controller,
        pos: Position,
    ) -> tuple[Direction, Build | None] | None:
        # Skip raiding if enemy has no meaningful economy (passive opponent).
        if not self.belief.en_harvesters and not self.belief.en_transport:
            return None
        best_tile = None
        best_flow = 0.0
        w = self.belief.w
        for i in self.belief.en_transport:
            if self.belief.en_flow.total[i] <= 0:
                continue
            ent = self.belief.entity[i]
            if ent is None or ent[0] not in _RAIDABLE:
                continue
            if i in self.belief.unit_tiles:
                continue
            if self.belief.en_flow.total[i] > best_flow:
                best_flow = self.belief.en_flow.total[i]
                best_tile = (i % w, i // w)
        if best_tile is None:
            return None
        target = Position(best_tile[0], best_tile[1])
        if pos == target:
            return Direction.CENTRE, Build(BuildKind.SELF_DESTRUCT, pos)
        move = self._move_toward(ct, pos, target)
        self._debug_target = (target, 255, 0, 255)
        return move, None
