...
class Builder(HarvestMixin, FixExcessMixin, FoundryMixin, RaidMixin, ExploreMixin):
    ...
    def _policy(self, ct: Controller, pos: Position) -> tuple[Direction, Build | None]:
        # 1. Fix disconnected harvesters/transport (highest priority).
        result = self._fix_excess_ti_rax(ct, pos)
        if result is not None:
            return result

        # 2. Place foundries to refine axionite.
        result = self._place_foundry(ct, pos)
        if result is not None:
            return result

        # 3. Fix ax excess (route to foundry or ti conveyor).
        result = self._fix_excess_ax(ct, pos)
        if result is not None:
            return result

        # 4. Harvest ore — ti first (primary resource), then ax.
        ...
        # 5. Raid enemy transport (only if they have infrastructure).
        result = self._raid(ct, pos)
        if result is not None:
            return result

        # 6. Explore unseen territory.
