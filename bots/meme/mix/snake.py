...
class Snake:
    ...
    def _init(self, p: Player, pos: Position) -> Generator:
        ...
        for _i in range(len(self.tail), self.n):
    ...
    def update(self, p: Player) -> Generator:
        ...
        assert p.apsp is not None
        assert p.pnb is not None
        assert p.core is not None
        assert turret is not None

        core_pos = [Position(core.x, core.y) for core in p.map.cores]
        friendly_core, enemy_core = (
            (core_pos[0], core_pos[1])
            if p.map.cores[0].team == p.team
            else (core_pos[1], core_pos[0])
        )
        ...
        if self.position.distance_squared(enemy_core) <= 20:
            ...
            unvisited = [
                ap for ap in attack_positions if (ap.x, ap.y) not in self.visited
            ]
            if not unvisited:
                return
            pos = self.position
            next_pos = min(
                unvisited,
                key=lambda ap: (
                    ap.distance_squared(pos),
                    -ap.distance_squared(enemy_core),
                ),
            )
        else:
            path = extract_path(
