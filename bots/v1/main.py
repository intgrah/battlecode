...
def find_nearby_allied_conveyor(ct: Controller, exclude: set) -> Position | None:
    pos = ct.get_position()
    my_team = ct.get_team()
    best = None
    best_dist = 999999
    for tile in ct.get_nearby_tiles():
        if (tile.x, tile.y) in exclude:
            continue
...
def bugnav_step(ct: Controller, target: Position, core_pos: Position | None, state: dict) -> bool:
    ...
    if not state.get("wall_following"):
        if _try_conv_step(ct, d, core_pos):
    ...
    for _ in range(8):
        if _try_conv_step(ct, scan, core_pos):
...
def _try_conv_step(ct: Controller, d: Direction, core_pos: Position | None) -> bool:
    pos = ct.get_position()
    next_pos = pos.add(d)
...
class CoreBot:
    ...
    def run(self, ct: Controller) -> None:
        ...
        if self.num_spawned == 0:
            if ti < bot_cost + harvester_cost:
                return
        elif ti < bot_cost + harvester_cost + 50:
            return
...
class BuilderBot:
    def __init__(self) -> None:
        ...
        self.sector_target: Position | None = None
    ...
    def _init(self, ct: Controller) -> None:
        ...
        self._pick_sector_target(ct)

    def _pick_sector_target(self, ct: Controller) -> None:
        if self.core_pos is None or self.sector_dir is None:
            return
        w = ct.get_map_width()
        h = ct.get_map_height()
        dx, dy = self.sector_dir.delta()
        self.sector_target = Position(
            max(1, min(w - 2, self.core_pos.x + dx * (w // 2))),
            max(1, min(h - 2, self.core_pos.y + dy * (h // 2))),
        )
        self.nav_state = {}
    ...
    def _economy(self, ct: Controller) -> None:
        ...
        if self.sector_target is not None and self.core_pos is not None:
            pos = ct.get_position()
            if pos.distance_squared(self.sector_target) <= 4:
                self.sector_dir = random.choice(DIRS)
                self._pick_sector_target(ct)
            bugnav_step(ct, self.sector_target, self.core_pos, self.nav_state)
            return
