...
def build_ground_truth(r, at_turn, team):
    ...
    for i, turn in enumerate(r.turns[:at_turn]):
...
def render_map(r, gt, at_turn, team, center=None, radius=8):
    ...
    for y in range(y0, y1):
        ...
        for x in range(x0, x1):
            ...
            elif pos in [h for h in gt["harvesters"]]:
                row += "H"
            else:
                env = r.map.rows[y].tiles[x] if y < len(r.map.rows) and x < len(r.map.rows[y].tiles) else 0
                if env == 1:
                    row += "#"
                elif env in (2, 3):
                    row += "o"
...
def main():
    ...
    if gt["breaks"]:
        ...
        for conv_pos, break_pos in gt["breaks"]:
            ...
            print(f"  Conv({conv_pos[0]},{conv_pos[1]}) -> gap({break_pos[0]},{break_pos[1]})  upstream_connected={conn}")
        print()

    print("=== Map (+ = connected, - = disconnected, X = break, H = harvester, C = core) ===")
    ...
    if disconnected:
        ...
        for pos, info in disconnected[:20]:
            print(f"  ({pos[0]},{pos[1]}) -> ({info['out'][0]},{info['out'][1]})  kind={info['kind']}")
