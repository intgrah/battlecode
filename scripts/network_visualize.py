...
foundries = []
for ai, ti in zip(ax_matched, ti_matched):
...
class Tree:
    ...
    def can_add_flow(self, path, amount):
        for eidx in path:
            if self.edge_flow[eidx] + amount > CAPACITY + 0.01:
                return False
        return True

    def increase_flow(self, path, amount):
        for eidx in path:
    ...
    def connect_source(self, pos, flow, commodity):
        if not self.edges:
            src = self.add_node(pos)
            self.add_edge(src, 0, flow, commodity)
            return True

        best_point, best_edge, best_t, best_dist = self.find_nearest_point(pos)
...
sources_by_dist = sorted(sources, key=lambda s: dist(s[0], core))
