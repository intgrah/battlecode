...
def _add_pascal_aliases(cls: type[Enum], names: dict[str, str]) -> None:
    """Add PascalCase aliases to an Enum class. Lets pyrust-translated
    bots use Rust-style variants (e.g. `Direction.North`) alongside the
    Python-style canonical names (`Direction.NORTH`)."""
    for pascal, canonical in names.items():
        setattr(cls, pascal, cls[canonical])
...
_add_pascal_aliases(
    EntityType,
    {
        ...
        "Conveyor": "CONVEYOR",
        "Splitter": "SPLITTER",
        "ArmouredConveyor": "ARMOURED_CONVEYOR",
        "Bridge": "BRIDGE",
        "Harvester": "HARVESTER",
        "Foundry": "FOUNDRY",
        "Road": "ROAD",
...
# The real Controller is injected from Rust at runtime, overwriting this placeholder.
# Keep the TYPE_CHECKING stub in sync with engine/rust/src/bindings/controller.rs.
#
# IMPORTANT: The runtime placeholder must have NO methods. With SHARED_GIL
# subinterpreters, Python methods on a Controller class pollute the Rust
# Controller's method resolution, causing bot-defined methods with the same
# name (e.g. `move`) to be incorrectly bound to Controller instances in other
# subinterpreters. The full stub is only for type checkers.
#
# NOTE: We use `if not TYPE_CHECKING: ... else: ...` (not a top-level class
# followed by a TYPE_CHECKING override) because mypy treats a top-level
# redefinition as an error and sticks with the first (empty) class.
