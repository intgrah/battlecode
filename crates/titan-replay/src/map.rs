...
use titan_core::connected::{
    CARDINALS, bridge_base_sprite_name as common_bridge_base, conveyor_sprite_name,
};
use titan_core::constants;
use titan_core::map::BG_COLOR;
use titan_core::tile::{premul, tile_center, tile_rect, tile_rect_f32};
    ...
    egui::CentralPanel::default().show_inside(ui, |ui| {
        ...
        if raw_scroll != 0.0 && response.hovered() {
            ...
            if let Some(pointer) = ui.input(|i| i.pointer.hover_pos()) {
                ...
                app.zoom = (app.zoom * factor).clamp(titan_core::tile::MIN_ZOOM, 10.0);
        ...
        app.pan = titan_core::tile::clamp_pan(
            app.pan,
            rect,
            app.game.width,
            app.game.height,
...
const fn team_str(team: proto::Team) -> &'static str {
    match team {
        proto::Team::A => "gold",
        proto::Team::B => "silver",
    }
...
fn bridge_base_sprite_name(
    ...
    let openings = collect_feeding_inputs(entity, by_pos, None);
    Some(common_bridge_base(team_str(entity.team), &openings))
}
...
fn conveyor_junction_sprite_name(
    ...
    let (base, out_dir) = match &entity.kind {
        EntityKind::Conveyor { dir, .. } => ("conveyor", *dir),
        EntityKind::ArmouredConveyor { dir, .. } => ("armoured_conveyor", *dir),
        _ => return None,
    };
    let out = dir_unit(out_dir)?;
    let inputs = collect_feeding_inputs(entity, by_pos, Some(out));
    conveyor_sprite_name(base, team_str(entity.team), out, &inputs)
}
