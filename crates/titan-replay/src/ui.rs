...
fn render_tagged_inline(ui: &mut egui::Ui, t: &Tagged, hover_tile: &mut Option<(i32, i32)>) {
    match t {
        ...
        Tagged::Tile(pos) => match pos {
            Some((x, y)) => {
                let text =
                    egui::RichText::new(format!("({x},{y})")).color(titan_core::style::COLOR_INFO);
    ...
    match node {
        LogNode::Vis { name, value } => {
            ...
            render_vis_row(
                ...
                name,
                value,
                live.map(std::convert::AsRef::as_ref),
                hover_tile,
                selected,
                hovered,
            );
...
const fn classify_row<'a>(
    inline: &'a Tagged,
    live: Option<&'a crate::vis::VisField>,
) -> RowKind<'a> {
...
fn render_key_value_row(
    ...
    ui.horizontal(|ui| {
        ui.monospace(egui::RichText::new(format!("{name}:")).color(titan_core::style::COLOR_KEY));
        match body {
