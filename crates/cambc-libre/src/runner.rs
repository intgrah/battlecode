...
use cambc_libre_engine::common::game_constants::MAX_TURNS;
use cambc_libre_engine::controller::UnitView;
use cambc_libre_engine::game::Game;
use cambc_libre_engine::game_map::Entity;
use cambc_libre_engine::replay_diff::GameDiff;
use cambc_libre_replay::map_loader;
...
impl GameRunner {
    ...
    fn run_turn(&mut self, py: Python, gc: &Bound<'_, PyModule>) -> PyResult<()> {
        ...
        if (game.turn + 1) % cambc_libre_engine::common::game_constants::PASSIVE_TITANIUM_INTERVAL == 0 {
            for p in &mut game.players {
                p.titanium += cambc_libre_engine::common::game_constants::PASSIVE_TITANIUM_AMOUNT;
            }
        }
...
pub fn run(args: Args) -> PyResult<MatchSummary> {
    Python::with_gil(|py| {
        ...
        cambc_libre_replay::write_replay(&game.replay_recorder, &args.replay, winner).map_err(|err| {
            pyo3::exceptions::PyIOError::new_err(format!(
