...
impl Paths {
    fn discover() -> Result<Self, String> {
        ...
        let crates_dir = harness_dir
            .parent()
            .ok_or("CARGO_MANIFEST_DIR has no parent")?
            .to_path_buf();
        let pyrust_root = harness_dir.clone();
        let translate_bin = if let Some(p) = std::env::var_os("PYRUST_TRANSLATE_BIN") {
            PathBuf::from(p)
        } else {
        ...
        let runner_root = crates_dir.join("pyrust-runner");
        let runner_manifest = runner_root.join("Cargo.toml");
        let runner_main = runner_root.join("src/main.rs");
        let runner_src_dir = runner_root.join("src");
        let corpus_dir = harness_dir.join("corpus");
        let check_dir = harness_dir.join("check");
        Ok(Self {
            pyrust_root,
            translate_bin,
            runner_manifest,
...
fn case_label(root: &Path, case_dir: &Path) -> String {
    case_dir.strip_prefix(root).ok().map_or_else(
