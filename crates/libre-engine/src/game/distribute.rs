...
impl Game {
    pub fn distribute_resources(&mut self) {
        // Each completed transfer produces a (source_pos, sink_pos,
        // resource_id) tuple for the replay's `DistributeResources` update.
        // resource_id is allocated from the global entity-id pool at the
        // moment a producer (harvester / foundry combine) issues a fresh
        // stack; storage transfers preserve the existing id.
        let mut moves: Vec<(Pos, Pos, i32)> = Vec::new();
        #[derive(Clone, Debug)]
        struct Edge {
            priority: f64,
            source: Pos,
...
stored: None | Some(ResourceType::Titanium | ResourceType::RawAxionite), .. })
                    ...
                    for sink_pos in entity.output_targets() {
                        ...
                        if sink_tile.building.is_some() {
                            count += 1;
                            incoming.entry(sink_pos).or_default().push(tile.position);
                        }
                    }
        ...
        for (sink_pos, sources) in &incoming {
            ...
            for source_pos in sources {
                ...
                let sink_can_accept = sink.can_accept_from(
                    resource,
                    *source_pos,
                    matches!(source, Entity::Bridge(_)),
                );
                if sink_can_accept {
        ...
        while let Some(edge) = heap.pop() {
            ...
            // Resolve the inbound stack id. Storage entities have one
            // already (via `feed_id`); a producing harvester gets a
            // freshly-allocated id from the global pool at this moment.
            let stored_feed_id = self
                .entities
                .get(&source_id)
                .expect("source missing")
                .feed_id();
            let in_id = stored_feed_id.unwrap_or_else(|| {
                self.next_id += 1;
                self.next_id
            });
                ...
                if let Some(up_resource) = upstream_resource {
                    let source_can_accept = {
                        ...
                        source.can_accept_from(
                            up_resource,
                            upstream_pos,
                            matches!(upstream, Entity::Bridge(_)),
                        )
                    };
