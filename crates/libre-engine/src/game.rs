...
use crate::common::game_constants::{
    ...
    GUNNER_VISION_RADIUS_SQ, HARVESTER_BASE_COST, HEAL_AMOUNT, LAUNCHER_BASE_COST,
    LAUNCHER_FIRE_COOLDOWN, LAUNCHER_VISION_RADIUS_SQ, MAX_TURNS, ROAD_BASE_COST,
    SENTINEL_AMMO_COST, SENTINEL_BASE_COST, SENTINEL_DAMAGE, SENTINEL_FIRE_COOLDOWN,
    SENTINEL_VISION_RADIUS_SQ, SPLITTER_BASE_COST, STARTING_AXIONITE, STARTING_TITANIUM,
};
...
use crate::replay_diff::{GameDiff, ReplayRecorder};
use paste::paste;
...
pub struct Game {
    ...
    /// True once any team called `Controller::resign` (with or without a
    /// message). Distinguishes "Resigned" from "core destroyed by enemy"
    /// in the match summary.
    pub resign_called: bool,
}
...
impl Game {
    ...
    pub fn winner_team(&mut self) -> Option<Team> {
        ...
        if self.turn >= MAX_TURNS || alive.is_empty() {
            ...
            // Tiebreak 6: coinflip
            return Some(if self.rng.random::<bool>() {
                Team::A
            } else {
                Team::B
            });
    ...
    /// Apply turret damage to a tile.
    ///
    /// Per `docs/spec/turrets.md`: "If a builder bot is standing on a
    /// building, turret attacks on that tile hit only the builder bot."
    /// So a bot on a tile shields any building from the same hit.
    pub fn damage_tile(&mut self, pos: Pos, amount: i32) {
        if !self.game_map.in_bounds(pos) {
            return;
        }
        let tile = self.game_map.tile(pos);
        if let Some(bot_id) = tile.builder_bot {
            self.apply_damage(bot_id, amount);
        } else if let Some(building_id) = tile.building {
            self.apply_damage(building_id, amount);
        }
    }
    ...
    pub fn remove_builder_bot(&mut self, id: i32) {
        ...
        self.players[bot.team.index()].scale_milli -= 200;
