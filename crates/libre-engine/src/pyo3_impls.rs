...
impl<'py> FromPyObject<'py> for EntityType {
    fn extract_bound(ob: &Bound<'py, PyAny>) -> PyResult<Self> {
        ...
        EntityType::from_str(&value)
            .map_err(|_| PyErr::new::<pyo3::exceptions::PyValueError, _>("invalid entity type"))
    }
}
...
impl<'py> IntoPyObject<'py> for EntityType {
    ...
    fn into_pyobject(self, py: Python<'py>) -> Result<Self::Output, Self::Error> {
        Ok(with_cache(|c| match self {
            EntityType::BuilderBot => c.et_builder_bot.clone_ref(py),
            EntityType::Core => c.et_core.clone_ref(py),
            EntityType::Gunner => c.et_gunner.clone_ref(py),
            EntityType::Sentinel => c.et_sentinel.clone_ref(py),
            EntityType::Breach => c.et_breach.clone_ref(py),
            EntityType::Launcher => c.et_launcher.clone_ref(py),
            EntityType::Conveyor => c.et_conveyor.clone_ref(py),
            EntityType::Splitter => c.et_splitter.clone_ref(py),
            EntityType::ArmouredConveyor => c.et_armoured_conveyor.clone_ref(py),
            EntityType::Bridge => c.et_bridge.clone_ref(py),
            EntityType::Harvester => c.et_harvester.clone_ref(py),
            EntityType::Foundry => c.et_foundry.clone_ref(py),
            EntityType::Road => c.et_road.clone_ref(py),
            EntityType::Barrier => c.et_barrier.clone_ref(py),
            EntityType::Marker => c.et_marker.clone_ref(py),
        })
        .into_bound(py))
    }
}
