...
pub fn emit_file(
    ...
    // Workspace-wide `#[pyrust::inline]` consts collected by `scan_inline_consts`.
    // Use sites resolve to the literal (LOAD_CONST), the declaration
    // is also emitted so cross-module imports keep working.
    for (name, lit) in &cfg.inline_consts {
        w.note_inline_const(name.clone(), lit.clone());
    }
    ...
    // Idempotent — safe to run once per imported file.
    for line in cambc_alias_prelude() {
        w.line(line);
    }
    w.blank_line();
...
fn required_imports(file: &syn::File, cfg: &CfgEnv) -> Result<Vec<String>, String> {
    ...
    // `type X = fn(...) -> ...` translates to `type X = Callable[..., ...]`
    // — emit the matching import so the alias resolves.
    let has_fn_type_alias = enabled_items.iter().any(|i| {
        if let syn::Item::Type(t) = i {
            matches!(*t.ty, syn::Type::BareFn(_))
        } else {
            false
        }
    });
    if has_fn_type_alias {
        out.push("from collections.abc import Callable".to_owned());
    }
...
fn cambc_alias_prelude() -> &'static [&'static str] {
    &[
