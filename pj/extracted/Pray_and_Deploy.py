...
Strategy:
  ...
  2. First 2 bots: build harvester + conveyor chain back to core for income.
...
class Player:
    ...
    def _core(self, ct: Controller) -> None:
        ...
        # Ramp up builder production over time
        want = min(2 + max(0, (rnd - 5) // 20), 10)
    ...
    def _init_builder(self, ct: Controller) -> None:
        ...
        # Assign first 2 bots to eco via marker counting
        eco_count = 0
    ...
    def _eco_plan(self, ct: Controller) -> None:
        ...
        # Scan for best titanium ore: close to core = shorter chain
        best_score = 999999
        best_ore = None
        for tile in ct.get_nearby_tiles():
        ...
        # Plan conveyor chain from ore toward core (ore-end first)
        chain = _plan_conveyor_chain(best_ore, core, self.w, self.h)

        # Build order: conveyors from core-adjacent outward, then harvester last.
        # Reversing means we build the conveyor closest to core first,
...
def _plan_conveyor_chain(harv, core, w, h):
    """Plan a cardinal-direction conveyor chain from harvester toward core.

    Uses only cardinal directions (N/S/E/W) for reliable 3-input/1-output behavior.
    At each step, moves along the axis with the larger remaining distance.
    ...
    for _ in range(30):
        ...
        # Pick cardinal direction reducing the larger axis difference
        if abs(dx) >= abs(dy):
        ...
        # If nxt is a core tile (Chebyshev dist <= 1 from center), chain is done
        if max(abs(nxt.x - core.x), abs(nxt.y - core.y)) <= 1:
            break

        # Compute this conveyor's facing direction (toward core from nxt)
        nxt_dx = core.x - nxt.x
        nxt_dy = core.y - nxt.y
        if abs(nxt_dx) >= abs(nxt_dy):
            nxt_d = Direction.EAST if nxt_dx > 0 else Direction.WEST
        else:
            nxt_d = Direction.SOUTH if nxt_dy > 0 else Direction.NORTH

        plan.append((nxt, nxt_d))
        pos = nxt
