...
@click.option(
    ...
    "use_platform",
    is_flag=True,
    help="Open map editor on platform instead of locally",
)
def map_editor(use_platform: bool) -> None:
    """Open the map editor.
...
def _serve_local() -> None:
    ...
    mono_dist = (
        Path(__file__).parent.parent.parent.parent.parent / "visualiser" / "dist"
    )
