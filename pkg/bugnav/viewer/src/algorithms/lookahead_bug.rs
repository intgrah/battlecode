...
fn dir_to_cell(from: (i32, i32), to: (i32, i32)) -> usize {
...
fn rotate_right(d: usize) -> usize {
    (d + 1) % 8
}
fn rotate_left(d: usize) -> usize {
    (d + 7) % 8
}
...
impl LookaheadBug {
    ...
    fn in_vision(&self, p: (i32, i32)) -> bool {
        dist_sq(self.pos, p) <= VISION_R_SQ
    }

    fn score_tile(&self, _p: (i32, i32)) -> i32 {
        0
    }
...
impl Pathfinder for LookaheadBug {
    ...
    fn name(&self) -> &'static str {
        if self.full_map { "LookaheadBug+FullMap" } else { "LookaheadBug" }
    }
}
